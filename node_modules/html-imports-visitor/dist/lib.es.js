...
import { getAttribute, predicates, queryAll } from 'dom5';

const isAbsoluteRegex = new RegExp("^([a-z]+://|//)", "i");

const resolve$1 = (basePath, href) => resolve(dirname(basePath), href);
const isLocal = (href) => href && !isAbsoluteRegex.test(href);
const load = (file) => readFileSync(file, "utf8");
const fileExists = (path$$1) => existsSync(path$$1);

const predicates$1 = predicates;
...
const traverse = (visitor, path$$1, predicate, importMap, link) => {
  ...
  const document = parse(load(path$$1));
...
class VisitorHelper {
   ...
   */
  omit(entry) {
    traverse(noopVisitor, resolve(entry), htmlImportPredicate, this.importMap);
    return this
  }
   ...
   */
  enter(entry) {
    traverse(this.visitor, resolve(entry), this.predicate, this.importMap);
    return this
  }

}

export { resolve$1 as resolve, isLocal, load, fileExists, predicates$1 as predicates, VisitorHelper };
